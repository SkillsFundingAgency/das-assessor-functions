...
namespace SFA.DAS.Assessor.Functions.Domain.Print
{
    public class PrintNotificationCommand : IPrintNotificationCommand
    {
        ...
        private async Task ProcessFile(PrintNotificationFileInfo file)
        {
            ...
            using (var validationService = new ValidationService(file.FileName, file.FileContent, _sftpSettings.ErrorPrintResponseDirectory, _fileTransferClient))
            {
                if (receipt?.Batch == null || receipt.Batch.BatchDate == DateTime.MinValue)
                {
                    validationService.Log(nameof(receipt.Batch), $"Could not process print notifications due to invalid file format [{file.FileName}]");
                ...
                {
                    validationService.Log(nameof(receipt.Batch.BatchNumber), $"Could not process print notifications the Batch Number is not an integer [{receipt.Batch.BatchNumber}] in the print notification in the file [{file.FileName}]");
                ...
                if (batch == null)
                {
                    validationService.Log(nameof(batch), $"Could not process print notifications unable to match an existing batch Log Batch Number [{batchNumberToInt}] in the print notification in the file [{file.FileName}]");
        ...
        private async Task ProcessLegacyFile(PrintNotificationFileInfo file)
        {
            ...
            using (var validationService = new ValidationService(file.FileName, file.FileContent, _sftpSettings.ErrorPrintResponseDirectory, _fileTransferClient))
            {
                ...
                {
                    validationService.Log(nameof(batchResponse.Batch), $"Could not process downloaded file due to invalid file format [{file.FileName}]");
                ...
                {
                    validationService.Log(nameof(batchResponse.Batch.BatchNumber), $"The Batch Number is not an integer [{batchResponse.Batch.BatchNumber}] in the print notification in the file [{file.FileName}]");
